...
bool USocketerBPLibrary::GetMessageFixedSize(USocket* Connection, int Size, FString &Message) {
	// If the passed in socket is not valid
	if (!IsValid(Connection))
	{
		return false;
	...
	*/
	...
	while (read_bytes < Size) {
		...
		MySocket->Recv(data.data() + read_bytes, data.size() - read_bytes, new_read_bytes);
	...
	data.push_back(0);
	// Convert it to an fstring and set the passed in message parameter
	Message = FString(ANSI_TO_TCHAR(reinterpret_cast<const char*>(data.data())));
	return true;
}
