...
class USocketerBPLibrary : public UBlueprintFunctionLibrary
{
	...
	* @return True if a message was successfully received, false otherwise.
	*/
	UFUNCTION(BlueprintCallable, meta = (DisplayName = "Get string of fixed size from server", Keywords = "Socketer send message tcpsend tcp tcpdisconnect socketersend fixed size"), Category = "Networking|Socketer")
		static bool GetMessageFixedSize(USocket* Connection, int Size, FString &Message);
